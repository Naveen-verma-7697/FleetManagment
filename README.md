...
- **Java backend** (`WanderCar_JAVA`) — Spring Boot 3.3, Spring Data JPA, Spring Security (BCrypt), JJWT, MySQL (embedded H2 also supported). Runs on port `8080`.
- **.NET backend** (`WanderCar_DotNet`) — ASP.NET Core 8, EF Core (Pomelo MySQL), FluentValidation, AutoMapper, Serilog, JWT + Google OAuth. Runs on port `5180`/`7156`. Both APIs expose the same route shapes, so the frontend needs no changes to switch between them.
